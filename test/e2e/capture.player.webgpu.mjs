// Attempts to capture PlayerPage with WebGPU enabled (headless Chrome flags).
// If WebGPU is unavailable, this script fails (to avoid writing misleading screenshots).
...
async function main() {
  ...
  const outPath = resolve('test', 'e2e', 'artifacts', 'player-webgpu.png')
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      const proc = spawn(
        ...
        [
          ...
          // Try to force WebGPU availability in headless.
