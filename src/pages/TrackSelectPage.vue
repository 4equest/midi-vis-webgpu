...
function onTrackEnabledChange(trackIndex: number, nextEnabled: boolean) {
  const s = appState.trackSettings.find((t) => t.trackIndex === trackIndex)
  if (!s) return
...
async function startPlayback() {
  ...
  if (!anyEnabled) {
    errorMessage.value = 'Select at least one track to display.'
    return
  }
  ...
  <div class="page">
    <div class="panel">
      ...
      <p v-if="midiSummary" class="muted" style="margin: 0 0 12px">
        Tracks: {{ midiSummary.tracks }} / Display: {{ midiSummary.enabled }} / Duration: {{ midiSummary.durationSeconds.toFixed(2) }}s
      </p>
          ...
          <div style="display: grid; gap: 8px; max-height: 240px; overflow: auto; padding: 6px 0">
            ...
            >
              <input
                type="checkbox"
                :checked="t.enabled"
                @change="onTrackEnabledChange(t.trackIndex, ($event.target as HTMLInputElement).checked)"
              />
              <div class="muted">
                {{ formatTrackLabel(t.trackIndex) }}
              </div>
